...
class Game:
    ...
    def get_mining_rate(self, turns = None, ship_id = None):
        ...
        #
        # estimate for minimal data
        #
        # estimates need to take into account ship param as well. I.e. we may have 100
        # 'mined' records, but no entries for a particular/new ship
        if len(self.game_metrics["mined"]) < 3:
            if not (ship_id is None):
                logging.warn("Nav  - Low data ship specific mining rate not implemented. t{}".format(self.turn_number))
        ...
        #
        # estimate an instantanious estimate from prior data
        #
    ...
    def get_mining_rate2(self, turns = None, ship_id = None):
        ...
        Use curve fitting to estimate - needs work/testing
        '''
        if len(self.game_metrics["mined"]) < 3:
            return self.get_mining_rate1(turns, ship_id)
        ...
        # oldest turn is aways 1 when using curve fitting
        oldest_turn = 1
        ...
        if ship_id is None:
            ...
            for t, halite in mined_by_turn.items():
                ship_count = self.game_metrics["ship_count"][i][1] if self.game_metrics["ship_count"][i][1] else 1
                mining_rate.append((t, halite / ship_count))

            #VALIDATE MINING RATE HERE
