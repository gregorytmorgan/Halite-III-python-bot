...
class GameMap:
    ...
    def move_cost(self, a, b, args={}):
        ...
        elif move_cost_type == "enemy":
            if player_id is None:
                raise RuntimeError("Miss required arg 'player_id' for move_cost_type enemy")
            cost = 0
            for pos in b.get_surrounding_cardinals():
                if pos == a:
                    continue
                cell = self[pos]
                cost += 1 if cell.is_occupied and cell.ship.owner != player_id else 0
            return cost
    ...
    def heuristic(self, start, current, goal, args = {}):
        ...
        elif move_cost_type == "danger":
            cost = 0
            for pos in current.get_surrounding_cardinals():
                cell = self[pos]
                cost += 1 if cell.is_occupied and cell.owner != me.id else 0
    ...
    def navigate(self, start, destination, algorithm="astar", args={}):
        ...
        :param destination Ending position
        :param algorithm:
          'astar' - takes args: 'move_cost': 'turns'|'halite'|'enemy
          'naive' - takes no args
          'dock' - takes no args
          'straightline' - takes no args
        """
        if algorithm == "astar":
            #move_cost = args["move_cost_type"] if "move_cost_type" in args else None
        ...
        elif algorithm == "danger":
            path, cost = self.get_danger_path(start, destination, args)
        elif algorithm == "dock":
    ...
    def get_danger_path(self, start, destination, args = {}):
        """
        ...
        ToDo - option to move away if there is currently an enemy beside the current position.
        """

        player_id = args["player_id"] if "player_id" in args else None

        if player_id is None:
            raise RuntimeError("Missing required argument 'player_id'")

        logging.debug("get_danger_path - s:{}, d:{}".format(start, destination))

        path = check_enemy_ships(self, start, destination, player_id, args)

        logging.debug("Nav  - Returning danger move {}".format(path))
    ...
    def get_docking_path(self, start, dropoff):
        ...
        if start.x == dropoff.x:
            ...
            path = [dropoff, Position(dropoff.x, start.y)]
